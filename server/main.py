...
def read_root(user_message: str):
    ...
    relevant_docs = retriever.invoke({"question": translated_to_english_user_message, "relevant_docs": []})
    print("   -> Finished")

    print("Asking the chatbot")
    response = llm.invoke({"question": translated_to_english_user_message, "relevant_docs": relevant_docs[:3]})
    print("   -> Finished")
